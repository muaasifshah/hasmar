...
const inputDirectory = path.join(cwd, 'svg-icons');
...
async function main() {
  try {
    ...
    if (files.length === 0) {
      ...
      await generateIconFiles(files, logVerbose);
    }
  } catch (error) {
...
async function generateIconFiles(files: string[], logVerbose: (message: string) => void) {
  ...
  const typeUpToDate = iconNames.every((name) => currentType.includes(`${name}`));
...
async function generateSvgSprite({ files, inputDirectory, outputDir }: { files: string[]; inputDirectory: string; outputDir: string }) {
